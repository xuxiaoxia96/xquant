...
func (s *StrategyParameter) QmtStrategyName() string {
	return QmtStrategyNameFromId(s.Id)
}

// Enable 策略是否有效
func (s *StrategyParameter) Enable() bool {
	return s.Auto && s.Id >= 0
}

// BuyEnable 获取可买入状态
func (s *StrategyParameter) BuyEnable() bool {
	return s.Enable() && s.Total > 0
}

// SellEnable 获取可卖出状态
func (s *StrategyParameter) SellEnable() bool {
	return s.Enable()
}

// IsCookieCutterForSell 是否一刀切卖出
func (s *StrategyParameter) IsCookieCutterForSell() bool {
	return s.SellEnable() && s.Total == 0
}

// NumberOfTargets 获得可买入标的总数
func (s *StrategyParameter) NumberOfTargets() int {
	if !s.BuyEnable() {
		return 0
	}
	return s.Total
...
func (s *StrategyParameter) initExclude() {
	if len(s.excludeCodes) > 0 {
		return
	}
	var excludeCodes []string
	for _, v := range s.Sectors {
...
func (s *StrategyParameter) StockList() []string {
	var codes []string
	for _, v := range s.Sectors {
		sectorCode := strings.TrimSpace(v)
		if !strings.HasPrefix(sectorCode, sectorIgnorePrefix) {
