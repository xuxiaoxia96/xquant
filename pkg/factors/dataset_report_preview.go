...
func (this *DataPreviewReport) Init(ctx context.Context, date string) error {
	...
	var allReports []east_money.PreviewQuarterlyReport
	// 确定更新日期
	qBegin, _ := api.GetQuarterDayByDate(date, 0)
	quarterBeginDate := exchange.FixTradeDate(qBegin)
	list, pages, _, _ := east_money.FinanceReports(quarterBeginDate, 1)
	...
	for pageNo := 2; pageNo < pages+1; pageNo++ {
		barSub.Add(1)
		list, pages, count, err := east_money.FinanceReports(quarterBeginDate, pageNo)
		if err != nil || pages < 1 {
			logger.Error(err)
			break
		}
		...
		allReports = append(allReports, list...)
		if count < east_money.EastmoneyFinanceReportsPageSize {
			break
		}
	}
	if len(allReports) > 0 {
